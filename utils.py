...
def segmentation_metrics(seg_truth, seg_predicted, two_pixel_threshold, samples=None, run_path=None):
    ...
    for i in range(n_samples):
        ...
        if samples is not None:
            ...
            plt.yticks([])
            plt.title('Image')
            plt.imshow(cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE))
            ...
            plt.yticks([])
            plt.title('Groundtruth')
            plt.imshow(cv2.rotate(seg_truth[i], cv2.ROTATE_90_CLOCKWISE), cmap='gray')
            ...
            plt.yticks([])
            plt.title('Segmentation')
            plt.imshow(cv2.rotate(seg_predicted[i], cv2.ROTATE_90_CLOCKWISE), cmap='gray', vmin=0, vmax=1)
            ...
            plt.yticks([])
            plt.title('Groundtruth\nDilated')
            plt.imshow(cv2.rotate(y_true_d, cv2.ROTATE_90_CLOCKWISE), cmap='gray', vmin=0, vmax=1)
            ...
            plt.imshow(cv2.rotate(y_pred, cv2.ROTATE_90_CLOCKWISE), cmap='gray', vmin=0, vmax=1)
